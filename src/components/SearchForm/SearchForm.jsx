...
const SearchForm = ({ name }) => {
  ...
  const { fromDate, toDate, fromCity, toCity } = useSelector(
    (state) => state.choice
  );
  ...
  const onClick = (e) => {
    dispatch(clearCity());
    show !== "city-from" && e.target.id === "from"
      ? setShow("city-from")
      : setShow("none");
    show !== "city-to" && e.target.id === "to"
      ? setShow("city-to")
      : setShow("none");
    if (city.from) {
      dispatch(getCity(city.from));
    ...
    <div className={`search-form ${name}`}>
      <div className={`search-form-container ${name}`}>
          ...
          <div className={`${show} ${name}`}>
            <div className="city-list">
              {items.length === 0 ? (
                ...
                items.map((el) => (
                  <p key={el._id} onClick={() => checkCity(el)}>
                    {el.name}
                  </p>
                ))
              )}
        ...
        <div className={`search-form-block block-date ${name}`}>
          ...
          {calendar.open && (
            <CalendarForm
              name={`main-page ${calendar.direction} ${name}`}
        ...
        <div className={`search-form-block block-controls ${name}`}>
          <div className="form-controls">
            <button className="form-controls_button" onClick={buttonClick}>
              Найти билеты
            </button>
          </div>
        </div>
