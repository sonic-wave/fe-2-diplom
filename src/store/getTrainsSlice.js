...
export const getTrains = createAsyncThunk(
  ...
  async (choice) => {
    ...
    const response = await axios(
      `https://students.netoservices.ru/fe-diplom/routes?from_city_id=${fromCity._id}&to_city_id=${toCity._id}&date_start=${fromDate}&date_end=${toDate}`
    );
    console.log(choice)
    console.log(response.data);
    return response.data;
...
export const getTrainsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(getTrains.pending, (state) => {
      state.loading = true;
    }); // Заменяем запятую на точку с запятой
